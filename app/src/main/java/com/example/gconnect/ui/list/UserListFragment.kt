...
//data class Connection(val id : String? =null, val userName: String? = null, val userEmail: String? = null,
//                      val headline: String? = null, val fullName: String? = null,
//                      val age: String?=null, val contact: String?=null, val addresses: String? = null,
//                      val education: String? = null, val skills: String? = null)
...
data class Notification( val eventId: String? = null, val fromUid : String? = null, val toUid: String? = null,
                        val fromEmail: String? = null, val message: String? = null, val sender: String? = null, val receiver : String? = null )
    ...
    private fun notification(selectedUser : Connection) {
        ...
        stdRef.addValueEventListener(object : ValueEventListener{
            override fun onDataChange(snapshot: DataSnapshot) {
                ...
                val Receiver = selectedUser.fullName
                val count = snapshot.child("Notification").childrenCount
//                Log.d("Count","count: $count")
                ...
                val individualUser = Notification(key ,auth.currentUser?.uid, selectedUser.id.toString()
                    ,auth.currentUser?.email,"$fullName is Trying to connect with you","$fullName", "$Receiver")
                notificationNode.child(individualUser.eventId.toString()).setValue(individualUser)
//                Log.d("CheckOnDataChange", "$individualUser")
            }
            ...
            override fun onDataChange(snapshot: DataSnapshot) {
                if(snapshot.exists()){
                   snapshot.children.forEach{
                       ...
                       //Log.d("CheckOnDataChange","$connectList")
                   }
                    rView.adapter = UserAdapter(tempArrayList, ::notification)
//                   Log.d("CheckOnDataChange", "1 : $connectList")
                }
            }
...
//        Log.d("CheckOnDataChange", "2 : $connectList")
